  ...
  if (findings.length === 0) {
    container.innerHTML = '' +
      '<div class="bg-white border border-gray-100 rounded-xl p-5 shadow-sm flex items-center gap-4">' +
        '<div class="h-12 w-12 rounded-full bg-white text-green-600 flex items-center justify-center flex-shrink-0">' +
          '<svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">' +
            '<path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />' +
          '</svg>' +
        '</div>' +
        '<div class="flex-1 min-w-0">' +
          '<h4 class="text-sm font-semibold text-gray-900 mb-0.5">Tidak ada masalah ditemukan</h4>' +
          '<p class="text-xs text-gray-500">Tidak ada kunci API, token, atau data sensitif yang terdeteksi di berkas proyek ini.</p>' +
        '</div>' +
      '</div>';
    return;
  }
